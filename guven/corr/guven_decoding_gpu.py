...
guven_behavior.ITEM_CATEGORY_DEG), so this uses mahal_discrete_basis_kfold_torch's
...
from mahal_discrete_basis_kfold_torch import (  # noqa: E402
    decode_discrete_basis_repetitions,
    decode_discrete_repetitions,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="hard",
        help=(
            "Use six hard category means directly, or convolve the balanced "
            "category means with a half-cosine basis before decoding"
        ),
    )
    parser.add_argument(
        "--basis-exponent",
        type=float,
        default=5.0,
        help="Exponent for --template-method basis (six categories -> 5)",
    )
    parser.add_argument(
        "--pool-sessions",
...
def main() -> None:
    args = parse_args()
    if args.template_method == "basis" and args.cv_method != "kfold":
        raise ValueError("--template-method basis requires --cv-method kfold")
    if args.basis_exponent <= 0:
        raise ValueError("--basis-exponent must be positive")
    ...
    if args.template_method == "basis":
        output_name = output_name.replace(
            "decoding_discrete_",
            f"decoding_discrete_basis{args.basis_exponent:g}_",
        )
    ...
    for subject in args.subjects:
            ...
            for name, column in (("cue", "cue_item"), ("uncue", "uncue_item")):
                ...
                if args.cv_method == "loocv":
                    ...
                    decoder = (
                        decode_discrete_basis_repetitions
                        if args.template_method == "basis"
                        else decode_discrete_repetitions
                    )
                    basis_kwargs = (
                        {"basis_exponent": args.basis_exponent}
                        if args.template_method == "basis" else {}
                    )
                    basis_label = (
                    ...
                    print(
                        f" pooled decoding {name}: 6-category "
                        f"{args.template_method} k-fold, "
                        f"{basis_label}"
            ...
            for entry in session_results:
                ...
                for name, column in (("cue", "cue_item"), ("uncue", "uncue_item")):
                    ...
                    if args.cv_method == "loocv":
                        ...
                        print(
                            f" session {entry['session']} decoding {name}: "
                            f"6-category {args.template_method} k-fold, "
                            f"{basis_label}"
                            f"n_folds={args.n_folds}, "
                            f"reps={args.reps}",
                            flush=True,
                        )
                        session_decoded[name] = decoder(
                            entry["features"],
                            theta,
                            config,
                            device=args.device,
        ...
        save_kwargs = dict(
            ...
            basis_exponent=(
                args.basis_exponent if args.template_method == "basis" else np.nan
            ),
            basis_function=(
                "row_normalized_(0.5+0.5*cos(theta-mu))^exponent"
                if args.template_method == "basis" else "none"
            ),

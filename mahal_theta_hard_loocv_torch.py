    ...
    for test_index in range(n_trials):
        ...
        if strata_t is not None:
            ...
            for stratum in torch.unique(strata_t):
                train_local_cpu = torch.where(
                    strata_t[train_cpu] == stratum
                )[0]
                ...
                sd = sd.clamp_min(torch.finfo(data_t.dtype).eps)
                train_data[train_local] = (
                    train_data[train_local] - mean
                ) / sd
        ...
        if completed % progress_step == 0 or completed == n_trials:
            print( f"  hard LOOCV trials {completed}/{n_trials}", flush=True, )

...
# Global default for trial selection in plotting/analysis functions.  Set this
# from a notebook (``twplot.RT_REMOVE = False``) to keep RT-invalid trials.
RT_REMOVE   = True
...
def resolve_line_idx(line_idx, default):
    ...
    """ 输入电极线的编号/名称，统一转换成idx， 0是最contra，10是最ipsi，5是midline """
    if line_idx is None:
        return list(default)
...
def _resolve_trial_mask(
    ...
    New Michael FFT pkl files retain every trial and store bad-trial status in
    ``is_bad_epoch``.  Those trials are excluded by default; pass
    ``exclude_bad_epoch=False`` only when they should deliberately be included.
    """
    mask = np.ones(len(d['early_rt']), dtype=bool)

    if exclude_invalid_rt is None:
        exclude_invalid_rt = RT_REMOVE
    ...
    if exclude_bad_epoch and "is_bad_epoch" in d:
        bad_epoch = np.asarray(d["is_bad_epoch"], dtype=bool).reshape(-1)
        if bad_epoch.size != mask.size:
            raise ValueError(
                "is_bad_epoch must have one entry per trial: "
                f"got {bad_epoch.size} flags for {mask.size} trials."
            )
        mask &= ~bad_epoch
...
def print_trial_summary(
    ...
    count), so they add up cleanly:
        ...
        total_removed% = bad_removed% + rt_removed% - overlap%

    For new FFT pkl files, ``total`` and ``removed_bad`` are read directly from
    the complete trial arrays and ``is_bad_epoch``.  Older pkl files that do
    not contain this flag retain the former raw-.mat fallback.

    total          = original trial count before exclusions.
    removed_bad    = trials marked by ``is_bad_epoch``.
    kept           = total - removed_bad.
    When ``exclude_invalid_rt`` is omitted, the global ``RT_REMOVE`` setting is
    used, matching the plotting functions.  With RT removal disabled, RT-based
    removal and overlap are reported as zero and all non-bad trials are kept.
    bad_removed%   = removed_bad / total.
    total_removed% = (bad_removed + rt_removed - overlap) / total.
    ...
    if exclude_invalid_rt is None:
        exclude_invalid_rt = RT_REMOVE
    has_bad_epoch_flags = all(
        "is_bad_epoch" in d
        for subj in subj_ids
        for d in by_subj[subj].values()
    )
    if raw_totals is None and not has_bad_epoch_flags:
    ...
    print("Trial summary (per subject, sessions pooled; all percentages are out of the "
          "raw total; total_removed% uses the union of bad and RT exclusions):")
    print("subj\ttotal\tkept%\tbad_removed%\trt_removed%\toverlap%\ttotal_removed%\tearly_rt_mean\tlate_rt_mean")
    ...
    for subj in subj_ids:
        ...
        for sess in sorted(by_subj[subj]):
            d = by_subj[subj][sess]
            if "is_bad_epoch" in d:
                n_total = len(d['early_rt'])
                bad_epoch = np.asarray(d["is_bad_epoch"], dtype=bool).reshape(-1)
                if bad_epoch.size != n_total:
                    raise ValueError(
                        f"subj={subj} sess={sess}: is_bad_epoch has "
                        f"{bad_epoch.size} entries for {n_total} trials."
                    )
                n_removed = int(bad_epoch.sum())
            else:
                n_total, n_removed = raw_totals[(subj, sess)]
                # Old pkl files already omit bad trials, so their overlap with
                # RT exclusion cannot be recovered from the cached arrays.
                bad_epoch = np.zeros(len(d['early_rt']), dtype=bool)
            n_kept = n_total - n_removed
            if "is_bad_epoch" not in d and n_kept != len(d['early_rt']):
                print(f"  [warn] subj={subj} sess={sess}: kept={n_kept} from raw bad_trials "
                      f"!= len(early_rt)={len(d['early_rt'])} in cached pkl")
            rt_valid = _rt_valid_mask(d, max_rt=max_rt) & _trial_filter_mask(d, trial_filter)
            if "has_timing_issue" in d:
                rt_valid &= ~np.asarray(d["has_timing_issue"], dtype=bool)
            if exclude_invalid_rt:
                # Final analysis set: valid RT/filter trials that are not bad epochs.
                analysis_keep = rt_valid & ~bad_epoch
                n_rt_removed = int(np.sum(~rt_valid))
                n_overlap = int(np.sum(bad_epoch & ~rt_valid))
            else:
                # RT is retained globally: only bad epochs are excluded.
                analysis_keep = ~bad_epoch
                n_rt_removed = 0
                n_overlap = 0
            n_rt_kept = int(np.sum(analysis_keep))
            subj_early_rts.append(np.asarray(d["early_rt"])[analysis_keep])
            subj_late_rts.append(np.asarray(d["late_rt"])[analysis_keep])

            subj_total   += n_total
            subj_removed += n_removed
            subj_rt_removed += n_rt_removed
            subj_overlap += n_overlap
            subj_kept_n  += n_kept
            subj_rt_kept += n_rt_kept

        total_trials      += subj_total
        total_removed_bad += subj_removed
        total_removed_rt += subj_rt_removed
        total_overlap += subj_overlap
        total_rt_kept     += subj_rt_kept

        bad_removed_pct   = 100.0 * subj_removed / subj_total if subj_total > 0 else 0.0
        rt_removed_pct    = 100.0 * subj_rt_removed / subj_total if subj_total > 0 else 0.0
        overlap_pct       = 100.0 * subj_overlap / subj_total if subj_total > 0 else 0.0
        total_removed_pct = 100.0 * (subj_removed + subj_rt_removed - subj_overlap) / subj_total if subj_total > 0 else 0.0
        ...
        subj_overlap_pct.append(overlap_pct)
        subj_total_removed_pct.append(total_removed_pct)
        early_rt_mean = float(np.nanmean(np.concatenate(subj_early_rts)))
        late_rt_mean = float(np.nanmean(np.concatenate(subj_late_rts)))
        ...
        print(f"{subj}\t{subj_total}\t{kept_pct:.2f}%\t{bad_removed_pct:.2f}%\t"
              f"{rt_removed_pct:.2f}%\t{overlap_pct:.2f}%\t{total_removed_pct:.2f}%\t"
              f"{early_rt_mean:.3f}\t{late_rt_mean:.3f}")

    if total_trials > 0:
        overall_bad_removed_pct   = 100.0 * total_removed_bad / total_trials
        overall_rt_removed_pct    = 100.0 * total_removed_rt / total_trials
        overall_overlap_pct       = 100.0 * total_overlap / total_trials
        overall_total_removed_pct = 100.0 * (total_removed_bad + total_removed_rt - total_overlap) / total_trials
        overall_kept_pct          = 100.0 * total_rt_kept / total_trials
    else:
        overall_bad_removed_pct = overall_rt_removed_pct = overall_overlap_pct = overall_total_removed_pct = overall_kept_pct = 0.0
    ...
    print(f"  overlap%         = {overall_overlap_pct:.2f}%")
    ...
    if subj_kept_pct:
        ...
        print(f"  overlap%       = {np.mean(subj_overlap_pct):.2f} ± {np.std(subj_overlap_pct, ddof=1):.2f}")
        print(f"  total_removed% = {np.mean(subj_total_removed_pct):.2f} ± {np.std(subj_total_removed_pct, ddof=1):.2f}")
        print(f"  early RT (s)   = {np.mean(subj_early_rt_means):.3f} ± {np.std(subj_early_rt_means, ddof=1):.3f}")
        print(f"  late RT (s)    = {np.mean(subj_late_rt_means):.3f} ± {np.std(subj_late_rt_means, ddof=1):.3f}")
    ...
    for subj in subj_ids:
        ...
        for sess in sorted(by_subj[subj]):
            ...
            for measure, arr in arrays.items():
                ...
                elif d["cue"][0] == 2: #late item on the left
                    arr = arr[::-1]

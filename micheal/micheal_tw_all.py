...
def load_and_compute_one(mat_path,
    ...
    # 所有 trial 均保留并参与 tw 计算，之前判定的 bad trial 仅通过
    # is_bad_epoch 标记，不再从数据中删除。
    ...
    is_bad_epoch = np.zeros(n_trials, dtype=bool)
    is_bad_epoch[bad_trials - 1] = True
    ...
    return fwmax, fwssmax, bwmax, bwssmax, fftf, time, ff, cue_loc, early_rot,late_rot, early_acc, early_rt, late_acc,late_rt, is_bad_epoch
...
def run_one_file(f):
    ...
    fwmax, fwssmax, bwmax, bwssmax, fftf, time, ff, cue_loc, \
        early_rot, late_rot, early_acc, early_rt, late_acc, late_rt, is_bad_epoch = load_and_compute_one(f)
